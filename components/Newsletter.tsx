...
const API_URL = process.env.NEXT_PUBLIC_NOTIFICATION_URL ?? ''

function sendGa4Event(email: string) {
  ...
  return (
    <section className="section-pad bg-[#080F24] relative overflow-hidden">
      ...
      <div className="relative max-w-2xl mx-auto px-6 text-center">
        ...
        {state === 'success' ? (
          ...
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            <input
              ...
              className="flex-1 bg-white/6 border border-white/12 text-white placeholder-white/30 px-5 py-3.5 rounded-full text-sm outline-none focus:border-[#1E6FFF]/60 focus:bg-white/8 transition-all"
            />
            <button
              type="submit"
        ...
        {/* Status messages */}
        {state === 'duplicate' && (
          <p className="mt-3 text-amber-400/80 text-sm">You're already subscribed — we'll keep the updates coming!</p>
        )}
        {state === 'error' && (
          <p className="mt-3 text-rose-400/80 text-sm">Something went wrong. Please try again in a moment.</p>
        )}
