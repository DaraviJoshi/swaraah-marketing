      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {REVIEWS.map((r) => (
            <div
              key={r.name}
              className="glass-card rounded-2xl p-6 flex flex-col hover:bg-white/8 transition-all duration-300 hover:-translate-y-1 group"
            >
              ...
              <div className="flex items-center justify-between mb-5">
                ...
                <Quote className="w-8 h-8 text-white/8 group-hover:text-white/12 transition-colors" />
              </div>
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-white/8">
                <div
                  className={`w-10 h-10 rounded-full bg-gradient-to-br ${r.avatarBg} flex items-center justify-center text-white text-xs font-bold shrink-0`}
                >
                  {r.avatar}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-white font-semibold text-sm">{r.name}</div>
                  <div className="text-white/40 text-xs truncate">{r.handle}</div>
                </div>
                <div className="text-right shrink-0">
                  <span className="text-lg">{r.emoji}</span>
                  <div className="text-white/30 text-xs mt-0.5">{r.trip}</div>
                </div>
              </div>
            </div>
          ))}
